...
- The location line (`formatWhere`) is `repo · task · space · tab · pane N`, and the desktop panel shows it as its heading with `status · Agent` underneath in small grey — which task it is matters more than what state it is in. The *task* segment is a linked worktree's own directory name (branch appended in parens only when it is not the same slug — `build_deploy_plan` vs `build-deploy-plan` counts as the same), or, for an ordinary checkout, the branch — suppressed on `main`/`master`/`trunk`, where there is no distinct task to name. `resolveWorktree` gets all of it from one `herdr worktree list --workspace <id>` call: `workspace get` only carries a `worktree` block for worktree-backed workspaces, so an ordinary repo used to show nothing but its workspace label. The space segment is dropped when it just repeats the repo or the task.
