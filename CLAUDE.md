...
- The location line (`formatWhere`) is `repo · worktree-name · space · tab · pane N`. The worktree segment appears only for linked worktrees (`is_linked_worktree`) and carries just the checkout's own name — no `worktree` prefix, and the branch is appended in parens only when it is not the same slug as that name (`build_deploy_plan` vs `build-deploy-plan` counts as the same). The space is dropped when it just repeats the repo or worktree name. The desktop panel shows this line as its heading, with `status · Agent` underneath in small grey — which task it is matters more than what state it is in.
